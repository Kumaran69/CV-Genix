...
import { createResume, updateResume, getResumeById } from "../services/api.js";
import { AuthContext } from "../context/AuthContext.jsx";
import {
  Save, Download, Eye, EyeOff, Palette, Sparkles,
  Undo2, CheckCircle, ArrowLeft, Clock, AlertCircle,
  Layers, Zap, FileText,
} from "lucide-react";
...
function formatDataForSave(data) {
  ...
  const name = p.firstName || p.lastName
    ...
    : p.name || "";
  const location = p.location || [p.address, p.city, p.country].filter(Boolean).join(", ");
  ...
  const experience = (data.experience || [])
    ...
    .map((ex) => ({
      role: ex.role, company: ex.company,
  ...
  return {
    title: data.title || name || "My Resume",
    template: data.template || "classic",
    personal: {
      name, email: p.email || undefined, phone,
  ...
  return (
    <button onClick={onClick}
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-300 ${
        active   ? "bg-blue-500/30 text-blue-100 border border-blue-400/60 scale-105"
        : filled ? "bg-blue-500/15 text-blue-200 border border-blue-400/35"
...
export default function ResumeBuilder() {
  ...
  // ✅ FIXED: reads selectedTemplate from location.state when returning
  // from TemplateSelection and applies it into resumeData immediately
  useEffect(() => {
    if (id) {
      const load = async () => {
        setIsFetching(true);
        ...
        try {
          ...
          const raw   =
            ...
            res.data?.data   ??
            res.data;
          if (!raw || typeof raw !== "object") throw new Error("Empty response from server.");
          const data = mergeResume(raw);
          // ✅ Apply template chosen in TemplateSelection if returning from it
          if (location.state?.selectedTemplate) {
            data.template = location.state.selectedTemplate;
          }
          setResumeData(data);
          setResumeId(id);
          ...
          setSaveError(
            err.response?.status === 404 ? "Resume not found. It may have been deleted." :
            err.response?.status === 401 ? "Session expired. Please log in again." :
            `Could not load resume: ${err.message}`
          );
        } finally {
          setIsFetching(false);
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (resumeId) {
        const res = await updateResume(resumeId, formatted, token);
        const updatedId =
          res.data?.resume?.id  || res.data?.resume?._id ||
          res.data?.id          || res.data?._id         || resumeId;
        ...
        const res = await createResume(formatted, token);
        const newId =
          res.data?.resume?.id  || res.data?.resume?._id ||
      ...
      setTimeout(() => { setShowSaveSuccess(false); navigate("/dashboard"); }, 2500);
      ...
      setSaveError(
        backendErrors
          ? (Array.isArray(backendErrors) ? backendErrors.join(", ") : backendErrors)
  ...
  // ✅ FIXED: passes resumeData correctly so TemplateSelection
  // receives it under location.state.resumeData as it expects
  const goToTemplates = () =>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center"
        ...
        <div className="text-center">
          ...
          <p className="text-blue-500/50 text-xs mt-1">ID: {id}</p>
        </div>
      </div>
    );
  }
        ...
        <header className="relative z-20 px-6 py-4" style={{ borderBottom:"1px solid var(--border)",backdropFilter:"blur(12px)" }}>
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="rb-display text-xl leading-tight font-bold" style={{ color:"#dbeafe" }}>CV&thinsp;·&thinsp;Genix</h1>
                <p className="text-xs leading-none mt-0.5" style={{ color:"#4a6080" }}>
                  {isEditing ? "Editing resume" : "New resume"}
                  {resumeData.personal.name && <span style={{ color:"#93c5fd" }} className="ml-1">— {resumeData.personal.name}</span>}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button onClick={handleUndo} disabled={history.length === 0}
                className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all disabled:opacity-30 disabled:cursor-not-allowed hover:bg-blue-500/10 active:scale-95"
                style={{ border:"1px solid var(--border)",color:"#93c5fd" }} title="Undo (Ctrl+Z)">
                <Undo2 className="w-4 h-4" /> Undo
              </button>
              <button onClick={goToTemplates}
                className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all hover:bg-blue-500/10 active:scale-95"
              ...
              <button onClick={handleSave} disabled={isSaving}
                className="pulse-ring flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold transition-all active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
          ...
          <div className="max-w-7xl mx-auto mt-4">
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {SECTIONS.map((s) => (
                <SectionPill key={s.key} label={s.label} icon={s.icon}
            ...
            {isEditing && (
              <p className="text-xs mt-2" style={{ color:"#4a6080" }}>
                {SECTIONS.every((s) => isSectionFilled(s.key, resumeData))
                  ? <span style={{ color:"#34d399" }}>✓ All sections complete — review and update as needed</span>
                  : <span>Continue from: <span style={{ color:"#93c5fd" }} className="font-medium">
        ...
        <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-8">
          {saveError && (
            <div className="animate-fade-in flex items-center gap-3 mb-6 px-5 py-3.5 rounded-xl text-sm"
              ...
              <button onClick={() => setSaveError("")} className="ml-auto text-red-400 hover:text-red-200 text-lg leading-none">×</button>
            </div>
          )}
              ...
              <div className="lg:sticky lg:top-6 h-fit animate-fade-in">
                <div className="rounded-2xl overflow-hidden"
                  ...
                  <div className="rb-scroll overflow-y-auto" style={{ maxHeight:"65vh" }}>
                    <div className="p-6"><ResumePreview resume={resumeData} /></div>
                  </div>
                  <div className="px-6 py-4 flex flex-col sm:flex-row gap-3" style={{ borderTop:"1px solid var(--border)" }}>
                    <button onClick={handleSave} disabled={isSaving}
                      className="pulse-ring flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold transition-all active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
                    ...
                    <button className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold hover:brightness-110 active:scale-95"
                ...
                <div className="mt-4 rounded-xl px-5 py-4"
                  ...
                  <p className="text-xs leading-relaxed" style={{ color:"#7eadd4" }}>
                    {completion<40?"Start with your name, title, and a short summary — recruiters read the top third first."
                    :completion<75?"Add at least 2 work experiences with quantified results to stand out."
